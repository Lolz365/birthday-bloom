...
const TreeSparks = ({ count, color }: {
    ...
    return (<div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
            {sparks.map((s) => (<motion.div key={s.id} initial={{ y: 0, opacity: 0 }} animate={{ y: -100, opacity: [0, 0.7, 0] }} transition={{ duration: s.duration, repeat: Infinity, delay: s.delay }} className="absolute rounded-full" style={{
                width: s.size,
                height: s.size,
                left: `${s.left}%`,
                bottom: `${s.bottom}%`,
...
export const HeartTree = ({ delay = 1000 }: HeartTreeProps) => {
    ...
    return (<div className="relative w-full max-w-[500px] aspect-square mx-auto overflow-hidden px-4 mb-20" style={{ perspective: "1000px" }}>
            <motion.div animate={{ rotateX: stage === 4 ? 20 : 0 }} className="relative w-full h-full preserve-3d">
                <div className={`absolute inset-0 transition-opacity duration-2000 pointer-events-none rounded-full blur-[100px]`} style={{
            ...
            return (<motion.g key={`leaf-${i}`} initial={{ scale: 0 }} animate={{ scale: stage >= 3 ? leaf.scale : 0 }} whileHover={{ scale: leaf.scale * 1.2, zIndex: 50 }} transition={{ type: "spring", stiffness: 200, damping: 10, delay: 3 + leaf.delay }} style={{ transform: `translate(${leaf.cx}px, ${leaf.cy}px)`, cursor: 'pointer' }} onClick={(e) => {
                    e.stopPropagation();
                    ...
                    setTimeout(() => setActiveMessage(null), 5000);
                                ...
                                {hasPhoto ? (<g>
                                    ...
                                    </g>) : (<path d={heartPath} fill={primaryColor} filter={stage === 4 ? "url(#treeGlow)" : ""} style={{
                ...
                <AnimatePresence>
                    {activeMessage && (<motion.div initial={{ opacity: 0, scale: 0.5, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.5, y: -20 }} className="absolute left-1/2 bottom-3/4 -translate-x-1/2 z-[100] w-[280px]">
