...
export const HeartTree = ({ delay = 1000 }: HeartTreeProps) => {
    ...
    const handleHeartClick = (leaf: typeof heartLeaves[0], quote: string) => {
        // Spawn a falling heart
        const id = Date.now();
        setFallingHearts(prev => [...prev, { id, cx: leaf.cx, cy: leaf.cy, color: primaryColor }]);
        setTimeout(() => setFallingHearts(prev => prev.filter(h => h.id !== id)), 1200);
        // Show message
    ...
    return (
        ...
        >
            {/* Intentional frosted-glass card — not an exposed div */}
            <div
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        {heartLeaves.map((leaf, i) => {
                            ...
                            return (
                                <motion.g
                                    ...
                                    onClick={(e) => { e.stopPropagation(); handleHeartClick(leaf, quote); }}
                                >
                                    {hasPhoto ? (
                                        ...
                                        <g transform={`translate(${leaf.cx}, ${leaf.cy})`}>
                                            <path d={heartPath} fill={primaryColor} filter={stage === 4 ? "url(#treeGlow)" : ""} style={{ animation: stage === 4 ? `pulse-scale 3s ease-in-out infinite alternate` : "none" }} />
                        ...
                        {/* Falling hearts on click — rendered inside SVG so coordinates match */}
                        {fallingHearts.map((fh) => (
                            <motion.path
                                key={fh.id}
                                d={heartPath}
                                fill={fh.color}
                                initial={{ opacity: 1, x: fh.cx, y: fh.cy, scale: 0.8 }}
                                animate={{ opacity: 0, y: fh.cy + 80, scale: 0.3, rotate: 30 }}
