...
export const PremiumFireworks = ({ runKey }: PremiumFireworksProps) => {
  ...
  useEffect(() => {
    if (!runKey || reduceMotion || typeof window === "undefined") return undefined;
    ...
    const launch = () => {
      if (!running || launched >= maxLaunches) return;
    ...
    const draw = (time: number) => {
      ...
      for (let i = particles.length - 1; i >= 0; i -= 1) {
        ...
        if (particle.life <= 0 || particle.y > window.innerHeight + 40) particles.splice(i, 1);
      }
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 z-[95] pointer-events-none mix-blend-screen"
      aria-hidden="true"
    />
  );
};
