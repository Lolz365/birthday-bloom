...
export const getHighlySpecificLetter = (name: string, relationship: RelationshipType, gender: GenderType, interests: string[] = []) => {
    ...
    if (relationship === 'friend') {
        if (isFemale)
            return EMOTIONAL_LETTERS.friend.friendly;
        if (isMale)
            return EMOTIONAL_LETTERS.friend.legend;
        return EMOTIONAL_LETTERS.friend.romantic;
    }
    ...
    if (relationship === 'colleague')
        return EMOTIONAL_LETTERS.colleague;
    if (relationship === 'mentor')
        return EMOTIONAL_LETTERS.mentor;
